...
def register() -> None:
    ...
    def list_voices() -> dict:
        return {"voices": [v.as_dict() for v in library.list()]}
    ...
    async def create_voice(
        ...
        try:
            voice = library.add(name, data, _suffix_for(file))
    ...
    async def speak(payload: dict = Body(...)) -> FileResponse:
        voice = library.get(str(payload.get("voice_id", "")))
        if not voice:
            raise HTTPException(404, "No such voice.")
        try:
            clip = await engine.synthesize(str(payload.get("text", "")), voice.sample_path)
